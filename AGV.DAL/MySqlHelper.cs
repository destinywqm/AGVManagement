   ...
   public class MySqlHelper
    {
        ...
        public static int ExecuteNonQuery(string SQLString)
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                using (MySqlCommand cmd = new MySqlCommand(SQLString, connection))
                {
                    ...
                    {
                        connection.Close();
                        MessageBox.Show("数据库错误:" + e.ToString());
                        return 0;
                    }
                }
            }
        ...
        public static DataTable ExecuteDataTableDelete(string SQLString)
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                DataSet ds = new DataSet();
        ...
        //接口调用日志函数
        public static void InsertTaskCompletionLog(long taskId, string status, bool isSuccess, string errorMessage = "")
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
...
CREATE TABLE IF NOT EXISTS TaskCompletionLog (
    ...
    ReportTime DATETIME DEFAULT CURRENT_TIMESTAMP
                    ...
                    using (MySqlCommand createCmd = new MySqlCommand(createTableSql, connection))
                    {
                    ...
                    // 插入日志
                    string insertSql = @"
INSERT INTO TaskCompletionLog (Id, Status, IsSuccess, ErrorMessage)
VALUES (@Id, @Status, @IsSuccess, @ErrorMessage);";

                    using (MySqlCommand insertCmd = new MySqlCommand(insertSql, connection))
                    {
                        insertCmd.Parameters.AddWithValue("@Id", taskId);
                        insertCmd.Parameters.AddWithValue("@Status", status);
                        insertCmd.Parameters.AddWithValue("@IsSuccess", isSuccess);
                ...
                {
                    MessageBox.Show("日志写入数据库失败: " + ex.Message);
                }
                finally
                {
                    connection.Close();
        ...
        public static DataTable ExecuteDataTableEnableNull(string SQLString)
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                DataSet ds = new DataSet();
                try
                {
                    ...
                    {
                        return new DataTable(); // 返回空表而不是抛异常
                    }
                }
                catch (MySql.Data.MySqlClient.MySqlException ex)
                {
