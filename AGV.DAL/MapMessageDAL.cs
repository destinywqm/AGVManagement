...
namespace AGV.DAL
{
    public class MapMessageDAL
    {
        ...
        {
            string sql = string.Format("SELECT COUNT(*) FROM agv.setting WHERE Map = '{0}'", mapName);
            DataTable dt = MySqlHelper.ExecuteDataTable(sql);

            // 如果返回的记录数大于 0，则表示地图存在
        ...
        /// <param name="Program"></param>
        /// <returns></returns>
        public bool DelRouteWithName(long MapTime, string MapName)
        {
            string sql = $"DELETE FROM agv.`route{MapTime}` WHERE `Name` = '{MapName}'";
        ...
        /// <param name="MapTime"></param>
        /// <returns></returns>
        public bool DelMapAndRelatedTables(long MapTime)
        {
            try
            {
                // SQL 删除语句
                string deleteMapSql = $"DELETE FROM agv.`map` WHERE `CreateTime` = {MapTime};";
                string deleteLineTableSql = $"DROP TABLE IF EXISTS agv.`line{MapTime}`;";
                string deleteDeviceTableSql = $"DROP TABLE IF EXISTS agv.`device{MapTime}`;";
                string deleteRouteTableSql = $"DROP TABLE IF EXISTS agv.`route{MapTime}`;";
                string deleteTagTableSql = $"DROP TABLE IF EXISTS agv.`tag{MapTime}`;";
                string deleteWidgetTableSql = $"DROP TABLE IF EXISTS agv.`widget{MapTime}`;";

                // 执行删除 map 表中 CreateTime = MapTime 的记录
                bool mapDeleted = MySqlHelper.ExecuteNonQuery(deleteMapSql) > 0;

                // 执行删除动态表
                MySqlHelper.ExecuteNonQuery(deleteLineTableSql);
                MySqlHelper.ExecuteNonQuery(deleteDeviceTableSql);
                MySqlHelper.ExecuteNonQuery(deleteRouteTableSql);
                MySqlHelper.ExecuteNonQuery(deleteTagTableSql);
                MySqlHelper.ExecuteNonQuery(deleteWidgetTableSql);

                // 如果 map 表的记录成功删除，则返回 true
                return mapDeleted;
            }
            catch (Exception e)
            {
                MessageBox.Show("数据库操作错误: " + e.ToString());
                return false;
            }
        }
        ...
        public bool DeleteAgvMap(List<string> sql)
        {
            return MySqlHelper.ExecuteSqlTran(sql);
        }
