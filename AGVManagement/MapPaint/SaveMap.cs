...
namespace AGVManagement.MapPaint
{
    public class SaveMap
    {
        ...
        private bool hasInserted = false;  // 标志是否已插入过新地图
        /// <summary>
        /// 保存地图信息
        /// </summary>
        ...
        /// <returns></returns>
        public bool SaveAtlas(string Times, bool type, string Name, double Width, double Height, string AgvStr, int MapType, double ActualWidth, double ActualHeight,  double Scale)
        {
            if (type && !hasInserted) // 
            {
                // 执行插入新地图操作
                InsertMapInfoNew(Name, Times, Width, Height, AgvStr, MapType, ActualWidth, ActualHeight, Scale);
                hasInserted = true; // 标志已插入，防止后续再次插入
            }
            else if (!type)
            {
                // 如果是编辑地图，执行更新操作
                UpdateMapNew(Name, AgvStr, Times, MapType, ActualWidth, ActualHeight, Scale);
            }
            else
            {
                // 如果已经插入过新地图，弹出提示
                MessageBox.Show("地图已经保存过，不能重复保存！");
                return false;
            }
            SaveTag(Times, type);
            InsertLine(Times);
        ...
        public void UpdateMapNew(string MapName, string AgvStr, string MapTime, int MapType, double ActualWidth, double ActualHeight, double Scale)
        {
            Sql.Add(string.Format(
                "UPDATE agv.`map` " +
                "SET `Name` = '{0}', `AGV` = '{1}', `Type` = {2}, `ActualWidth` = {3}, `ActualHeight` = {4}, `Scale` = {5} " +
                "WHERE CreateTime = {6}",
                MapName, AgvStr, MapType, ActualWidth, ActualHeight, Scale, MapTime));
        }

        public void DeleteMap(string MapName, string MapTime)
        {
        ...
        public void InsertMapInfoNew(string Name, string CreateTime, double Width, double Height, string AgvStr, int MapType, double ActualWidth, double ActualHeight, double Scale)
        {
            Sql.Add(string.Format(
        "INSERT INTO agv.`map` (`Name`, `CreateTime`, `Width`, `Height`, `AGV`, `Type`, `ActualWidth`, `ActualHeight`, `Scale`) " +
        "VALUES ('{0}', {1}, {2}, {3}, '{4}', {5}, {6}, {7}, {8})",
        Name, CreateTime, Width, Height, AgvStr, MapType, ActualWidth, ActualHeight, Scale));
        }
        ...
        private void SaveTag(string CreateTime, bool type)
        {
            ...
            /*foreach (var kvp in MapInstrument.tagLogicPositions)
            {
                ...
                sqlTagd.Append(strTag);
                sqlTagd.Append("('");
                sqlTagd.Append("TA" + tagId);
                sqlTagd.Append("','");
                sqlTagd.Append(tagId);
                sqlTagd.Append("',");
                sqlTagd.Append((pos.X + 19) / 10); // 逻辑X坐标，单位米
                sqlTagd.Append(",");
                sqlTagd.Append((pos.Y + 11.5) / 10); // 逻辑Y坐标，单位米
